...
public class Gv350mTextProtocolDecoder extends BaseProtocolDecoder {

    private final boolean ignoreFixTime;

    public Gv350mTextProtocolDecoder(Protocol protocol) {
        super(protocol);
    ...
    private static final Pattern PATTERN_INF = new PatternBuilder()
            ...
            .number("(d{1,2}),")                 // rssi
            .number("d{1,2},")
            .expression("[01]{1,2},")            // external power supply
            .number("d{0,5},")                   // external power vcc
            .number("x{4},")                     // INF Expand Mask
            ...
            .number("[013]")                   // external gps antenna
            .number("d{0,5},")                   // analog input voltage
            .number("x{2,4}")                    // digital input
            .number("x{2,4}")                    // digital output
            .number("(d*){0,8},")                     // wire device number
            .groupBegin()
    ...
    private static final Pattern PATTERN_LOCATION = new PatternBuilder()
            ...
            .number("(-?d{1,3}.d{0,6})?,")         // longitude
            .number("(-?d{1,2}.d{0,6})?,")         // latitude
            .number("(dddd)(dd)(dd)")            // date (yyyymmdd)
